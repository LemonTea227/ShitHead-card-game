...
def main():
    ...
    try:
        ...
        send_receive_thread = threading.Thread(target=async_send_receive, args=(sock,))
...
def open_screen(event, pos):
    ...
    quick_game = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 75, 700, 100, 'Quick Game')
    ...
    choose_a_room = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 440, 700, 100, 'Choose A Room')
    create_a_room = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 590, 700, 100, 'Create A Room')
...
def settings_menu(event, pos):
    ...
    quick_game_settings = button.Button(PINK, WINDOW_WIDTH / 2 - 350, 75, 700, 100, 'Quick Game Settings')
    number_of_players = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 200, 700, 100, 'Number Of Players')

    minus = load_image(MOVE_LEFT, PINK)
    screen.blit(minus, (WINDOW_WIDTH / 2 - 110 - 50 - 220, WINDOW_HEIGHT / 2 - 115))
    ...
    number = button.Button(WHITE, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT / 2 - 50, 100, 100, str(num))
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
        if WINDOW_WIDTH / 2 - 110 - 50 - 220 < pos[0] < WINDOW_WIDTH / 2 - 110 - 50:
            if WINDOW_HEIGHT / 2 - 115 < pos[1] < WINDOW_HEIGHT / 2 - 115 + 230:
                if num > 2:
                    num -= 1
                    with open('preferences.txt', 'w') as f:
                        f.write(str(num))
                    # number = button.Button(WHITE, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT / 2 - 50, 100, 100,
                    #                       str(num))

        elif WINDOW_WIDTH / 2 + 110 + 50 < pos[0] < WINDOW_WIDTH / 2 + 110 + 50 + 220:
            if WINDOW_HEIGHT / 2 - 115 < pos[1] < WINDOW_HEIGHT / 2 - 115 + 230:
                if num < 4:
                    num += 1
                    with open('preferences.txt', 'w') as f:
...
def create_a_room_menu(event, pos, num):
    ...
    create_a_room_button = button.Button(PINK, WINDOW_WIDTH / 2 - 350, 75, 700, 100, 'Create A Room')
    ...
    create = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, WINDOW_HEIGHT - 150, 700, 100, 'Create')
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
        if create.is_over(pos):
            ...
            # print 'apply'
        elif WINDOW_WIDTH / 2 - 110 - 50 - 220 < pos[0] < WINDOW_WIDTH / 2 - 110 - 50:
            if WINDOW_HEIGHT / 2 - 115 < pos[1] < WINDOW_HEIGHT / 2 - 115 + 230:
                if num > 2:
                    num -= 1
                    number = button.Button(WHITE, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT / 2 - 50, 100, 100,
...
def choose_a_room_menu(event, pos, games_message, page):
    ...
    choose_a_room = button.Button(PINK, WINDOW_WIDTH / 2 - 350, 75, 700, 100, 'Choose A Room')
    red_raw_window(choose_a_room)

    last_page = load_image(MOVE_LEFT, PINK)
    screen.blit(last_page, (WINDOW_WIDTH / 2 - 110 - 50 - 220, WINDOW_HEIGHT - 230))

    num = page
    number = button.Button(WHITE, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT - 150, 100, 100, str(num))
    red_raw_window(number)
    ...
    if len(games) >= (num - 1) * 5 + 1:
        room1 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 200, 700, 100,
                              'ROOM ' + str(games[(num - 1) * 5].num_room + 1) + ' Online: ' + str(
                                  games[(num - 1) * 5].players) + '/' + str(
    ...
    if len(games) >= (num - 1) * 5 + 2:
        room2 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 302, 700, 100,
                              'ROOM ' + str(games[(num - 1) * 5 + 1].num_room + 1) + ' Online: ' + str(
                                  games[(num - 1) * 5 + 1].players) + '/' + str(
    ...
    if len(games) >= (num - 1) * 5 + 3:
        room3 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 404, 700, 100,
                              'ROOM ' + str(games[(num - 1) * 5 + 2].num_room + 1) + ' Online: ' + str(
                                  games[(num - 1) * 5 + 2].players) + '/' + str(
    ...
    if len(games) >= (num - 1) * 5 + 4:
        room4 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 506, 700, 100,
                              'ROOM ' + str(games[(num - 1) * 5 + 3].num_room + 1) + ' Online: ' + str(
                                  games[(num - 1) * 5 + 3].players) + '/' + str(
    ...
    if len(games) >= (num - 1) * 5 + 5:
        room5 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 608, 700, 100,
                              'ROOM ' + str(games[(num - 1) * 5 + 4].num_room + 1) + ' Online: ' + str(
                                  games[(num - 1) * 5 + 4].players) + '/' + str(
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
        ...
        if room1.is_over(pos):
            if len(games) >= (num - 1) * 5 + 1 and games[(num - 1) * 5].players != \
                    games[(num - 1) * 5].max_players:
                choice = games[(num - 1) * 5].num_room
                do_choose = True

        elif room2.is_over(pos):
            if len(games) >= (num - 1) * 5 + 2 and games[(num - 1) * 5 + 1].players != \
                    games[(num - 1) * 5 + 1].max_players:
                choice = games[(num - 1) * 5 + 1].num_room
                do_choose = True

        elif room3.is_over(pos):
            if len(games) >= (num - 1) * 5 + 3 and games[(num - 1) * 5 + 2].players != \
                    games[(num - 1) * 5 + 2].max_players:
                choice = games[(num - 1) * 5 + 2].num_room
                do_choose = True

        elif room4.is_over(pos):
            if len(games) >= (num - 1) * 5 + 4 and games[(num - 1) * 5 + 3].players != \
                    games[(num - 1) * 5 + 3].max_players:
                choice = games[(num - 1) * 5 + 3].num_room
                do_choose = True

        elif room5.is_over(pos):
            if len(games) >= (num - 1) * 5 + 5 and games[(num - 1) * 5 + 4].players != \
                    games[(num - 1) * 5 + 4].max_players:
            ...
            if WINDOW_HEIGHT - 230 < pos[1] < WINDOW_HEIGHT:
                if num >= 2:
                    ...
                    if len(games) >= (num - 1) * 5 + 1:
                        room1 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 200, 700, 100,
                                              'ROOM ' + str(
                                                  games[(num - 1) * 5].num_room + 1) + ' Online: ' + str(
                                                  games[(num - 1) * 5].players) + '/' + str(
                                                  games[(num - 1) * 5].max_players))
                    else:
                        room1 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 200, 700, 100,
                    ...
                    if len(games) >= (num - 1) * 5 + 2:
                        room2 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 302, 700, 100,
                                              'ROOM ' + str(
                                                  games[(num - 1) * 5 + 1].num_room + 1) + ' Online: ' + str(
                                                  games[(num - 1) * 5 + 1].players) + '/' + str(
                                                  games[(num - 1) * 5 + 1].max_players))
                    else:
                        room2 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 302, 700, 100,
                    ...
                    if len(games) >= (num - 1) * 5 + 3:
                        room3 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 404, 700, 100,
                                              'ROOM ' + str(
                                                  games[(num - 1) * 5 + 2].num_room + 1) + ' Online: ' + str(
                                                  games[(num - 1) * 5 + 2].players) + '/' + str(
                                                  games[(num - 1) * 5 + 2].max_players))
                    else:
                        room3 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 404, 700, 100,
                    ...
                    if len(games) >= (num - 1) * 5 + 4:
                        room4 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 506, 700, 100,
                                              'ROOM ' + str(
                                                  games[(num - 1) * 5 + 3].num_room + 1) + ' Online: ' + str(
                                                  games[(num - 1) * 5 + 3].players) + '/' + str(
                                                  games[(num - 1) * 5 + 3].max_players))
                    else:
                        room4 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 506, 700, 100,
                    ...
                    if len(games) >= (num - 1) * 5 + 5:
                        room5 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 608, 700, 100,
                                              'ROOM ' + str(
                                                  games[(num - 1) * 5 + 4].num_room + 1) + ' Online: ' + str(
                                                  games[(num - 1) * 5 + 4].players) + '/' + str(
                                                  games[(num - 1) * 5 + 4].max_players))
                    else:
                        room5 = button.Button(WHITE, WINDOW_WIDTH / 2 - 350, 608, 700, 100,
...
def wait_to_full(event, pos, room, p_now, people):
    ...
    if not p_now == people:
        ...
        pn = button.Button(WHITE, WINDOW_WIDTH - 200 - 2, 2, 200, 100, str(p_now) + '/' + str(people))
...
def game_manager(event, pos, cards_message, cards_to_throw, to_who):
    ...
    if deck > 0:
        deck_card = cards.Cards(None, None, WINDOW_WIDTH / 2 + 50, WINDOW_HEIGHT / 2 - 125 + 10)
        deck_button = button.Button(WHITE, WINDOW_WIDTH / 2 + 50 + 200 + 2, WINDOW_HEIGHT / 2 - 50 + 10, 100, 100,
                                    str(deck))
    ...
    if deck_top_number != 0:
        top_card = cards.Cards(deck_top_number, deck_top_shape, WINDOW_WIDTH / 2 - 250, WINDOW_HEIGHT / 2 - 125 + 10)
    ...
    if hand:
        ...
        for i in range(len(hand)):
            ...
            for card in cards_to_throw:
                if card.get_number() == hand[i][0] and card.get_shape() == hand[i][1]:
                    in_throw = True
            if in_throw:
                hand[i] = cards.Cards(hand[i][0], hand[i][1], start_draw_hand, WINDOW_HEIGHT - 90 - 10)
            else:
                hand[i] = cards.Cards(hand[i][0], hand[i][1], start_draw_hand, WINDOW_HEIGHT - 90)
            start_draw_hand += 40
    ...
    secrets = []
    if secret == 1:
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 100 - 20, WINDOW_HEIGHT - 92 - 250))
    elif secret == 2:
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 250 - 20, WINDOW_HEIGHT - 92 - 250))
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 + 50 - 20, WINDOW_HEIGHT - 92 - 250))
    elif secret == 3:
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 350 - 20, WINDOW_HEIGHT - 92 - 250))
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 100 - 20, WINDOW_HEIGHT - 92 - 250))
        secrets.append(cards.Cards(None, None, WINDOW_WIDTH / 2 + 150 - 20, WINDOW_HEIGHT - 92 - 250))
    for card in secrets:
        card_to_window(card)
    ...
    if len(visible) == 1:
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[0][0] and card.get_shape() == visible[0][1]:
                in_throw = True
        if in_throw:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 100,
                                     WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT - 92 - 250)
    elif len(visible) == 2:
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[0][0] and card.get_shape() == visible[0][1]:
                in_throw = True
        if in_throw:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 250,
                                     WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 250, WINDOW_HEIGHT - 92 - 250)
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[1][0] and card.get_shape() == visible[1][1]:
                in_throw = True
        if in_throw:
            visible[1] = cards.Cards(visible[1][0], visible[1][1], WINDOW_WIDTH / 2 + 50, WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[1] = cards.Cards(visible[1][0], visible[1][1], WINDOW_WIDTH / 2 + 50, WINDOW_HEIGHT - 92 - 250)
    elif len(visible) == 3:
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[0][0] and card.get_shape() == visible[0][1]:
                in_throw = True
        if in_throw:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 350,
                                     WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[0] = cards.Cards(visible[0][0], visible[0][1], WINDOW_WIDTH / 2 - 350, WINDOW_HEIGHT - 92 - 250)
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[1][0] and card.get_shape() == visible[1][1]:
                in_throw = True
        if in_throw:
            visible[1] = cards.Cards(visible[1][0], visible[1][1], WINDOW_WIDTH / 2 - 100,
                                     WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[1] = cards.Cards(visible[1][0], visible[1][1], WINDOW_WIDTH / 2 - 100, WINDOW_HEIGHT - 92 - 250)
        in_throw = False
        for card in cards_to_throw:
            if card.get_number() == visible[2][0] and card.get_shape() == visible[2][1]:
                in_throw = True
        if in_throw:
            visible[2] = cards.Cards(visible[2][0], visible[2][1], WINDOW_WIDTH / 2 + 150,
                                     WINDOW_HEIGHT - 92 - 250 - 10)
        else:
            visible[2] = cards.Cards(visible[2][0], visible[2][1], WINDOW_WIDTH / 2 + 150, WINDOW_HEIGHT - 92 - 250)
    for card in visible:
        card_to_window(card)

    throw_button = button.Button(WHITE, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT / 2 + 10, 100, 100, 'T')
    red_raw_window(throw_button)

    turn = int(cards_message[5].split(':')[1]) + 1
    turn_button = button.Button(RED, WINDOW_WIDTH / 2 - 50, WINDOW_HEIGHT / 2 - 100 + 10, 100, 100, str(turn))
    ...
    for part in cards_message[6:]:
        if part.split(':')[0] == 'Player':
            ...
            for card in cards_str:
                if card != '':
                    vis.append((int(card.split(',')[0]), int(card.split(',')[1])))
    ...
    if len(other_players_data) >= 1:
        num_cards_player_buttons.append(
            button.Button(WHITE, WINDOW_WIDTH / 2, 2, 100, 100, str(other_players_data[players_num[0]][0])))
        num_player_buttons.append(button.Button(PINK, WINDOW_WIDTH / 2 - 100, 2, 100, 100, str(players_num[0])))
        lst_secret = []
        if other_players_data[players_num[0]][2] >= 1:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 350 - 20, 112))
        if other_players_data[players_num[0]][2] >= 2:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 100 - 20, 112))
        if other_players_data[players_num[0]][2] == 3:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 + 150 - 20, 112))
        lst_visible = []
        if len(other_players_data[players_num[0]][1]) >= 1:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[0]][1][0][0], other_players_data[players_num[0]][1][0][1],
                            WINDOW_WIDTH / 2 - 350, 112))
        if len(other_players_data[players_num[0]][1]) >= 2:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[0]][1][1][0], other_players_data[players_num[0]][1][1][1],
                            WINDOW_WIDTH / 2 - 100, 112))
        if len(other_players_data[players_num[0]][1]) == 3:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[0]][1][2][0], other_players_data[players_num[0]][1][2][1],
                            WINDOW_WIDTH / 2 + 150, 112))
        players_cards_secret.append(lst_secret)
        players_cards_visible.append(lst_visible)

    if len(other_players_data) >= 2:
        num_cards_player_buttons.append(
            button.Button(WHITE, 2, WINDOW_HEIGHT / 2 - 52, 100, 100,
                          str(other_players_data[players_num[1]][0])))
        num_player_buttons.append(
            button.Button(PINK, 2, WINDOW_HEIGHT / 2 + 52, 100, 100, str(players_num[1])))
        lst_secret = []
        if other_players_data[players_num[1]][2] >= 1:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 500 - 200 - 20 - 20,
                                          WINDOW_HEIGHT / 2 - 125 - 250 - 20 - 20))
        if other_players_data[players_num[1]][2] >= 2:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 - 500 - 200 - 20 - 20, WINDOW_HEIGHT / 2 - 125))
        if other_players_data[players_num[1]][2] == 3:
            lst_secret.append(
                cards.Cards(None, None, WINDOW_WIDTH / 2 - 500 - 200 - 20 - 20, WINDOW_HEIGHT / 2 + 125 + 20 + 20))
        lst_visible = []
        if len(other_players_data[players_num[1]][1]) >= 1:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[1]][1][0][0], other_players_data[players_num[1]][1][0][1],
                            WINDOW_WIDTH / 2 - 500 - 200 - 20, WINDOW_HEIGHT / 2 - 125 - 250 - 20 - 20))
        if len(other_players_data[players_num[1]][1]) >= 2:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[1]][1][1][0], other_players_data[players_num[1]][1][1][1],
                            WINDOW_WIDTH / 2 - 500 - 200 - 20, WINDOW_HEIGHT / 2 - 125))
        if len(other_players_data[players_num[1]][1]) == 3:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[1]][1][2][0], other_players_data[players_num[1]][1][2][1],
                            WINDOW_WIDTH / 2 - 500 - 200 - 20, WINDOW_HEIGHT / 2 + 125 + 20 + 20))
        players_cards_secret.append(lst_secret)
        players_cards_visible.append(lst_visible)

    if len(other_players_data) >= 3:
        num_cards_player_buttons.append(
            button.Button(WHITE, WINDOW_WIDTH - 102, WINDOW_HEIGHT / 2 - 52, 100, 100,
                          str(other_players_data[players_num[2]][0])))
        num_player_buttons.append(
            button.Button(PINK, WINDOW_WIDTH - 102, WINDOW_HEIGHT / 2 + 52, 100, 100, str(players_num[2])))
        lst_secret = []
        if other_players_data[players_num[2]][2] >= 1:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 + 500 + 20 - 20,
                                          WINDOW_HEIGHT / 2 - 125 - 250 - 20 - 20))
        if other_players_data[players_num[2]][2] >= 2:
            lst_secret.append(cards.Cards(None, None, WINDOW_WIDTH / 2 + 500 + 20 - 20, WINDOW_HEIGHT / 2 - 125))
        if other_players_data[players_num[2]][2] == 3:
            lst_secret.append(
                cards.Cards(None, None, WINDOW_WIDTH / 2 + 500 + 20 - 20, WINDOW_HEIGHT / 2 + 125 + 20 + 20))
        lst_visible = []
        if len(other_players_data[players_num[2]][1]) >= 1:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[2]][1][0][0], other_players_data[players_num[2]][1][0][1],
                            WINDOW_WIDTH / 2 + 500 + 20, WINDOW_HEIGHT / 2 - 125 - 250 - 20 - 20))
        if len(other_players_data[players_num[2]][1]) >= 2:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[2]][1][1][0], other_players_data[players_num[2]][1][1][1],
                            WINDOW_WIDTH / 2 + 500 + 20, WINDOW_HEIGHT / 2 - 125))
        if len(other_players_data[players_num[2]][1]) == 3:
            lst_visible.append(
                cards.Cards(other_players_data[players_num[2]][1][2][0], other_players_data[players_num[2]][1][2][1],
                            WINDOW_WIDTH / 2 + 500 + 20, WINDOW_HEIGHT / 2 + 125 + 20 + 20))
        players_cards_secret.append(lst_secret)
        players_cards_visible.append(lst_visible)
    ...
    if event.type == pygame.MOUSEBUTTONDOWN and event.button == LEFT:
        ...
        if throw_button.is_over(pos):
            if cards_to_throw:
                message += 'THROW~'
                for card in cards_to_throw:
                    message += str(card.get_number()) + ',' + str(card.get_shape()) + '|'
                message += '~'
                message += str(to_who)
                message += '~~~'
                send = True
        if top_card and top_card.is_over(pos):
            ...
            for cr in reversed(hand):
                if cr.is_over(pos):
                    if not cards_to_throw or cards_to_throw[0].get_number() == cr.get_number():
                        can_remove = True
        ...
        if hand:
            for cr in reversed(hand):
                if cr.is_over(pos):
                    if cards_to_throw and cards_to_throw[0].get_number() == cr.get_number():
                        can_remove = False
                        card_to_remove = None
                        for c in cards_to_throw:
                            if c.get_shape() == cr.get_shape():
...
def event_handler(scrn, pos, event, more_args):
    ...
    if scrn == 'OPEN_SCREEN':
        ...
        return wait_to_full(event, pos, more_args[0], more_args[1], more_args[2])
    elif scrn == 'GAME':
        return game_manager(event, pos, more_args[0], more_args[1], more_args[2])
    elif scrn == 'FINISH':
